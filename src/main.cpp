...
void doInference(IExecutionContext& context, float* input, void* output, const int output_size, cv::Size input_shape) {
...
int main(int argc, char** argv) {
    ...
    for (int j = 0; j < out_dims.nbDims; ++j)
    {
        ...
        output_size *= abs(out_dims.d[j]);

    }
    
    std::cout<< "==> prob begin "<<std::endl;
    // void*  prob = (void *)new char[output_size];
    float* prob = new float[output_size];
    // float* prob = (float*) malloc(sizeof(float) * output_size);
    std::cout<< "==> prob end "<<std::endl;
